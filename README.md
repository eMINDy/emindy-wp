...
Developers should use feature branches and Pull Requests when making changes (see docs/development.md and docs/contributing.md for guidelines). All code must adhere to WordPress coding standards and project-specific best practices (accessibility, i18n, security). Commit messages should be clear and prefixed by component (e.g. theme: ... or plugin: ...). Continuous Integration is set up to package the theme and plugin into ZIPs for deployment – refer to docs/installation.md and docs/RELEASE.md for the deployment workflow.
...
Ensure you have a WordPress environment (v6.x or higher recommended) with the Twenty Twenty-Five parent theme installed (for the child theme to activate).

Clone this repository into the WordPress wp-content directory, or download and extract the latest built ZIPs for the theme and plugin.

Activate the eMINDy Core plugin and eMINDy theme via the WordPress admin dashboard.

Refer to docs/installation.md for detailed setup steps (including any required plugins such as Polylang for multilingual support).

After activation, configure the site as described in docs/configuration.md (create essential pages, set up language settings, etc.) to match the expected structure of the platform.

For a high-level understanding of the project’s mission and architecture, see docs/overview.md and docs/architecture.md. These will provide context on how the theme and plugin work together to deliver the eMINDy mental wellness platform.
