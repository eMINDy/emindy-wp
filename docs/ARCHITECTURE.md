...
Default seeding translates labels while keeping sanitised slugs so repeat runs or
language changes do not create duplicates.
Default terms are inserted for each taxonomy, and all are public and REST-enabled. The seeding is idempotent via `term_exists()`, so deployments can safely re-run registration without duplicating rows.【F:wp-content/plugins/emindy-core/includes/class-emindy-taxonomy.php†L73-L100】
...
Registration shares a single capability callback so only editors can mutate the
REST-exposed meta fields.
...
- **Practice & assessment**:
  ...
  - `[em_phq9]`, `[em_gad7]` – PHQ-9 and GAD-7 self-check questionnaires with scoring (see JavaScript handling in assets).【F:wp-content/plugins/emindy-core/includes/class-emindy-shortcodes.php†L170-L221】
  - `[em_assessment_result]` – Displays assessment summary based on query parameters, verifying an HMAC signature derived from the auth salt to block tampered scores.【F:wp-content/plugins/emindy-core/includes/class-emindy-shortcodes.php†L574-L623】
...
- `[em_assessment_result]` – Displays assessment summary based on signed query parameters; an HMAC using the WordPress auth salt blocks tampering before rendering the score band.【F:wp-content/plugins/emindy-core/includes/class-emindy-shortcodes.php†L596-L635】
...
- **Discovery & related content**:
  - `[em_related]` – Related content grid using shared taxonomies with language-aware queries and a deterministic fallback search built from the current post title/excerpt (no user input).【F:wp-content/plugins/emindy-core/includes/class-emindy-shortcodes.php†L141-L214】
  - Helpers for popular content, sitemap-like lists and report links appear alongside related logic (within the same class).
- **Search & utility**:
  - Search UI helpers such as filters, search bar snippets and quick filters live alongside other utility shortcodes (e.g., transcript helpers, table of contents, share links) within the class file; search terms are sanitised before being passed into `WP_Query`. Lightweight “popular” queries set `no_found_rows` to avoid pagination overhead when embedded repeatedly.【F:wp-content/plugins/emindy-core/includes/class-emindy-shortcodes.php†L780-L931】
- **Newsletter**:
