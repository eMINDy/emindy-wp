...
class Schema {
    ...
    public static function build_exercise_howto_schema( WP_Post $post ) {
        ...
        $schema = [
            ...
            'publisher'    => [ '@id' => esc_url_raw( $org_id ) ],
            'isPartOf'     => [ '@id' => esc_url_raw( $site_id ) ],
            'image'        => $image ? [ '@type' => 'ImageObject', 'url' => esc_url_raw( $image ) ] : null,
            'step'         => $steps,
    ...
    public static function build_article_schema( WP_Post $post ) {
        ...
        $schema = [
            ...
            'inLanguage'       => $in_lang,
            'image'            => $image ? [ '@type' => 'ImageObject', 'url' => esc_url_raw( $image ) ] : null,
            'author'           => $author ? [ '@type' => 'Person', 'name' => wp_strip_all_tags( $author ) ] : null,
    ...
    public static function output_jsonld() {
        ...
        printf(
            ...
            esc_html( wp_json_encode( $schema, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_HEX_TAG ) )
        );
    }
}
